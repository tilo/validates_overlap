...
**Ranges must be linear (`start <= end`) — cyclic/wraparound semantics are fundamentally incompatible with this validator.** On a circular domain such as time-of-day, every pair of values denotes *some* valid range (`11:00..10:00` is simply the 23-hour complement of `10:00..11:00`), so a wraparound window is indistinguishable from accidentally swapped fields — no validation can tell intent from typo. Pure time-of-day columns (`t.time`) therefore work for windows within a single day only; a window crossing midnight (`22:00..02:00`) must be split into two records, or use datetime columns. Beware that `'24:00'` is cast to `00:00`. To catch inverted ranges loudly instead of silently, pair the overlap validation with an order check on your model, e.g. `validates :ends_at, comparison: { greater_than: :starts_at }`.
