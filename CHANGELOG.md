...
### Improvements
  ...
  - README: indexing guidance — the overlap check runs an `EXISTS` query on every save; add a composite index (scope columns first, then the range columns) to avoid table scans; the PostgreSQL exclusion constraint doubles as a suitable GiST index
