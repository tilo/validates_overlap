...
### Improvements
  ...
  - the overlap check works on any orderable column type — now covered by specs for date, datetime, timestamp, time, integer, decimal, and string range columns (including open-ended ranges and integer gap/tolerance shifts) and documented in the README ("non-date ranges")
...
### Internal
  ...
  - removed the constants `OverlapValidator::BEGIN_OF_UNIX_TIME` and `END_OF_UNIX_TIME` — the sentinel substitution they existed for is gone
